...
from vacancy.infrastructure.persistence.tables import setup_mapping
from vacancy.presentation.web.controllers import (
    HEALTHCHECK_CONTROLLER,
    SOURCE_CONTROLLER,
    VACANCY_CONTROLLER,
)
from vacancy.presentation.web.exceptions.setup import (
...
def custom_openapi(application: FastAPI) -> Any:
    ...
    openapi_schema = get_openapi(
        title="Vacancy",
        summary="Vacancy management API",
        version=f"{version}",
...
def setup_controllers(application: FastAPI) -> None:
    ...
    application.include_router(SOURCE_CONTROLLER)
    application.include_router(VACANCY_CONTROLLER)
