...
class IncrementFailedLoginCommand(Command[None]):
    user_id: UserId


class IncrementFailedLoginCommandHandler(CommandHandler[IncrementFailedLoginCommand, None]):
    def __init__(
        self,
    ...
    async def handle(self, command: IncrementFailedLoginCommand) -> None:
        ...
        await self.__logger.ainfo(
            event="INCREMENT_FAILED_LOGIN_COMMAND",
            user_id=str(command.user_id),
            attempts=credential.failed_login_attempts,
        )
