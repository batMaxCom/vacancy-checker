...
from auth.entrypoint.di.containers import web_container
from auth.entrypoint.web.config import AppConfig, get_web_config
from auth.infrastructure.adapters.logger import setup_logging
from auth.infrastructure.persistence.tables import setup_mapping
from auth.presentation.web.controllers import (
    AUTH_CONTROLLER,
...
def setup_controllers(application: FastAPI) -> None:
    ...
    application.include_router(AUTH_CONTROLLER)
    application.include_router(REGISTRATION_CONTROLLER)
    application.include_router(INTROSPECTION_CONTROLLER)
