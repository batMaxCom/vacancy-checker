...
from search.entrypoint.di.containers import web_container
from search.entrypoint.web.config import AppConfig, get_web_config
from search.infrastructure.adapters.logger import setup_logging
from search.infrastructure.persistence.tables import setup_mapping
from search.presentation.web.controllers import (
    HEALTHCHECK_CONTROLLER,
    SEARCH_JOB_CONTROLLER,
    SEARCH_PROFILE_CONTROLLER,
)
from search.presentation.web.exceptions.setup import (
...
def setup_application(config: AppConfig) -> FastAPI:
    ...
    application.openapi = lambda: custom_openapi(application)  # type: ignore[method-assign, unused-ignore]
    return application
...
def setup_controllers(application: FastAPI) -> None:
    ...
    application.include_router(SEARCH_JOB_CONTROLLER)
    application.include_router(SEARCH_PROFILE_CONTROLLER)
